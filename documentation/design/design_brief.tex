...
\abstract{%
   Dual Tone Multi Frequency (DTMF for short) encoding is a technology used for devices (including servers) to communicate with other devices over a standard analogue telephone line. [citation needed]
   This is mainly used for automated switchboards for calls; however, this is also used in dial up modems, call menus, remote control systems, telephone banking, and other applications. [citation needed]
   
   The technology functions by converting each of 16 symbols on a keypad (1-9, A-D, * and \#) into a special tone. The tone consists of a sum of two sine waves, whose frequency is determined
   by pairwise combination of 4 high frequencies (representing the keypad columns) and 4 low frequencies (representing the keypad rows).
   
   The frequencies used fall within the same range as the human voice, allowing the tones to be transmitted over telephone lines as audible sounds. 
   The tone is received by a decoder, and its spectrum is analysed to determine the keypad symbol pressed.

   This document outlines the design of a DTMF encoding system based on a microcontroller board. The system described makes use of an LCD display to show the user 
   what keys they have pressed as well as to display any messages or errors. An LED is also be used as an indicator of the system's state. 
   An on-board DAC, in conjunction with an amplifier circuit and speaker are used to generate and amplify the DTMF tones.
   }
...
\section{Introduction}
   ...
   Microcontroller programming poses several technical challenges. Microcontrollers are designed for use in real-time systems, and hence do not have an operating system which deals with the issue 
   of task management. Task scheduling has to be handled explicitly by the programmer, through the use of an ad-hoc scheduler and/or interrupt handlers.

   In addition, microcontrollers are generally low-cost systems with limited hardware resources, and hence require the use of low-level programming tricks to achieve efficiency and performance
